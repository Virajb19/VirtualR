...
export default function Navbar() {
   ...
   useGSAP(() => {
    ...
    return <>
         ...
         <button onClick={() => setMobileDrawerOpen(!mobileDrawerOpen)} className='hidden mb:inline mr-1'>{mobileDrawerOpen ? <X /> : <Menu />}</button>
      </nav>
      {mobileDrawerOpen && (
         <div ref={drawerRef} className='hidden w-full fixed z-20 mb:block text-white bg-[#161616] space-y-3'>
            <ul className='text-xs flex justify-around p-3'>
              {navItems.map((item,i) => {
                return <li key={i} className='border border-transparent hover:border-zinc-300 p-1 rounded-full'>
                   <a href={item.href}>{item.label}</a>
                </li>
              })}
            </ul>
